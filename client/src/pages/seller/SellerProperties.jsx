...
import { CheckCircle2, ExternalLink, Plus, RefreshCw, Send, Trash2 } from 'lucide-react';
...
export default function SellerProperties() {
  ...
  async function deleteListing(id) {
    ...
    try {
      ...
      const message = err?.response?.data?.error || err?.message || 'Failed to delete listing';
      setError(message);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Create, submit, and manage your listings.</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Draft a property and optionally submit it for approval.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {items.map((p) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {p.rejectionReason ? <div className="text-xs text-destructive">Rejection: {p.rejectionReason}</div> : null}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="outline" onClick={() => deleteListing(p.id)}>
